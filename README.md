...
**分割中だけ**、各ペインの上に高さ18pxのヘッダが出る（1ペインのときは出ない）。既定では `zsh` / `claude` / `claude (再開)` のような種別と、そのペインの cwd のディレクトリ名（basename）を示す。
...
分割で作るペインは常にシェルで、cwd を分割元から引き継ぐため、**名前を付けるまで分割した2枚のヘッダは同じ文字列になる**（`zsh・my-repo` が2つ並ぶ）。どちらで何をしていたか分からなくなるので、長く残すペインには名前を付けておくとよい。
...
| `Cmd+J` / `Cmd+Shift+J` | 次 / 前の「あなたの番」のタブへジャンプ | ウィンドウ |
| `Cmd+,` | 設定ウィンドウを開く | アプリ名 |
