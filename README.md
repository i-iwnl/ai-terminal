...
- **動作中の AI** — tmux でのラップ（「アプリを閉じても AI の作業を続ける」）、**このフォルダのものだけ表示する**、更新間隔
...
**確認ダイアログが出るのは2つの場合だけ。** (1) 2つ以上のペインを一度に閉じるとき、(2) 1つでも**再開先を特定できないもの**があるとき（通常は起きない。CLI が古くてセッション ID を渡せなかった場合などの縮退）。「アプリを閉じても AI の作業を続ける」が有効なペインは、claude / gemini とも閉じてもプロセスが残り**履歴から再開できる**ので止めない。この判定は `Cmd+W`・`Cmd+Option+W`・タブバーの x ボタン・メニューの「タブを閉じる」で共通。何本のプロセスが終わるかは x ボタンのラベル（ポインタを合わせたときの説明と読み上げ名の両方）に出る。

閉じたときにバックグラウンドへ残ったものは、**閉じた直後に読み上げで伝える**（「タブを閉じました。AI の作業 1 件は終了せず残っています。Gemini 1 件はサイドバーの「履歴」から再開できます…」）。
...
**「アプリを閉じても AI の作業を続ける」を有効にしていると、タブを閉じてもプロセスが残る**
