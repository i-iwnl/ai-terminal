...
macOS の「システム設定 > アクセシビリティ > ディスプレイ > コントラストを上げる」に追従する。**アプリ側の設定は要らない。** 有効にすると、弱い文字が1段濃くなり、一覧の区切り線・選択中のタブ・分割中のアクティブなペインの枠線がはっきりする。タブの状態マーク（「あなたの番」の丸・「異常終了」の四角）も明るくなる。
