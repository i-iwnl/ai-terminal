...
`Cmd+T` か「+」ボタンで新しいシェルタブが開く。タブごとに独立したシェルで、`Cmd+W` で閉じられる（最後の1枚を閉じると新しいシェルが自動的に開く）。**タブの中をさらに分割している場合、`Cmd+W` はアクティブなペインだけを閉じる。タブそのものが閉じるのは、そのタブに残っているペインが1枚だけのとき**（「ターミナルを分割する」を参照）。
