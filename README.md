...
**確認ダイアログが出るのは2つの場合だけ。** (1) 2つ以上のプロセスが同時に終了するとき、(2) 1つでも**閉じると二度と開き直せないもの**があるとき（「アプリを閉じても AI の作業を続ける」を有効にしている gemini のペイン。claude は履歴から再開できるので止めない）。この判定は `Cmd+W`・`Cmd+Option+W`・タブバーの x ボタン・メニューの「タブを閉じる」で共通。何本のプロセスが終わるかは x ボタンにポインタを合わせると事前に分かる。
...
**⚠ gemini を走らせたまま「履歴」で gemini を選ぶと、その走行中セッションの記録が消える**

履歴一覧は `gemini --list-sessions` を実行して作っている。**このコマンドは走行中の gemini セッションを一覧に出さないうえ、その記録ファイル（`~/.gemini/tmp/<プロジェクト>/chats/*.jsonl`）を削除する**（2026-08-06 / Gemini CLI 0.53.0 で実測。会話済みでも消え、以後そのプロセスが書き足しても復活しない）。**現時点の回避策は「gemini タブを走らせている間は履歴一覧で gemini を選ばない」だけ**（claude 側は無関係で、いつ開いても安全）。アプリ側の対処は未着手。
...
> **gemini が戻れない理由は「セッション ID を採番できないから」ではない**（2026-08-06 / Gemini CLI 0.53.0 で実測）。`gemini --session-id <UUID>` は存在し、渡した UUID はそのまま `--list-sessions` の一覧に出る。それでも戻れないのは、**`gemini --list-sessions` が走行中のセッションを一覧に出さない**うえ、**実行するとその走行中セッションの記録ファイルを削除してしまう**ため。tmux で生き残らせた gemini はまさに走行中なので、履歴に1行も現れず選べない。
