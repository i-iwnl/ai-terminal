...
3つのパネル（タスク / 履歴 / メモ）は、それぞれ最上部に**いまどの範囲を見ているか**を常に表示する（「このマシン全体の Claude」「このフォルダの Claude」「このアプリのメモ」など）。3つは似た見た目で並んでいるが対象範囲がまったく違う — とくに**タスク一覧は既定でマシン全体**なので、このアプリで起動していないセッションも混ざる。0件のときもこの行は消えない。
