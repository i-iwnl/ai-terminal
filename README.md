...
**タブの左端には状態のマークが出る。** 丸（オレンジ）は「そのタブの AI があなたの入力を待っている」、四角はプロセスが終了したタブ。形が違うので色を見分けられなくても区別できる。選択中のタブは下辺の白い線で示す。

**分割しているタブでは、この2つのマークの出る条件が違う。** 丸は**どれか1つのペイン**が待っていれば出る（裏のペインで待たれていることに気づけるように）。四角は**すべてのペインが終了したときだけ**出る（1枚でも生きていれば、そのタブを選べばまだ入力できるため）。どちらもアクティブなペインだけを見てはいないので、`Cmd+]` でペインを移してもマークは動かない。
...
macOS の「システム設定 > アクセシビリティ > ディスプレイ > コントラストを上げる」に追従する。**アプリ側の設定は要らない。** 有効にすると、弱い文字が1段濃くなり、一覧の区切り線・選択中のタブ・分割中のアクティブなペインの枠線がはっきりする。タブの状態マーク（「あなたの番」の丸・「終了」の四角）も明るくなる。
