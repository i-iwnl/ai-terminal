...
**ウィンドウの位置・サイズ・フルスクリーン状態は次回起動時に復元される**（外部ディスプレイを外したあとなど、保存された位置が画面外になる場合は中央に開き直す）。ウィンドウのタイトルはアクティブなタブの名前に追従するので、複数のプロジェクトを別ウィンドウで開いていても Mission Control やウィンドウメニューで見分けられる。**開いていたタブそのものは復元しない**（プロセスを勝手に起動し直さない）。
