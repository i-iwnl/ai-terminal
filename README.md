...
**押せる行には左端に細い縦線が出る。** 一覧には押せる行（開いているタブへ移動する / タブに戻す）と押せない行（このアプリの外で起動されたセッションなど）が混ざるので、**ポインタを載せなくても、どの行が操作できるかが見える**。線のぶんは内側の余白から差し引いてあるため、押せる行と押せない行で文字の位置はずれない。

**押せる行を右クリックすると「この AI を終了」が出る。** タブを開き直さなくても、一覧から直接 AI を終了できる（[#244](https://github.com/i-iwnl/ai-terminal/issues/244)）。**いま作業中の AI を終了しようとしたときだけ確認が出る**（タブを閉じるときと同じ考え方で、終わっている AI は確認なしで即座に終了する）。終了しても**会話はサイドバーの「履歴」から再開できる**。戻らないのは実行中だった作業だけ。

**マウスを使わずに終了することもできる。** Tab で行まで辿ると白い枠が出るので、そのまま「ファイル > 選択中の AI を終了」を選ぶ。対象は**タスク一覧で最後に選んだ行**で、対象が無いあいだ項目は淡色（無効）のまま。終了して対象が消えれば、また淡色に戻る。

> 右クリックのメニューが出るのは**押せる行だけ**。押せない行は、終了できる対象（生きているセッション）がそもそも無いので出ない。終了して行が消えると、キーボードのフォーカスは**同じ節に残っている行**へ移り、その節が空になったらパネルの見出しへ移る。
...
> 右クリックが効く場所はもう1つある。サイドバーの**タスク一覧の押せる行**で、そちらは「この AI を終了」が出る（[実行中タスクを一覧で見る](#5-実行中タスクを一覧で見る)）。
...
**AI を残したまま片付けたいときは、メニューの「ファイル > AI を残してタブを閉じる」を使う。** ショートカットは割り当てていない（`Cmd+Shift+W` は「ウィンドウを閉じる」）。こちらで閉じたセッションはサイドバーの「タスク」に残り、行を押せばタブに戻せる。**あとから終了したくなったら、その行を右クリックして「この AI を終了」**（タブを開き直す必要は無い）。

**確認ダイアログが出るのは3つの場合だけ。** (1) 2つ以上のペインを一度に閉じるとき、(2) **いま作業中の AI を止めるとき**（終わった AI を閉じるのが大半なので、日に1〜3回程度）、(3)「AI を残して閉じる」で、1つでも**再開先を特定できないもの**があるとき（通常は起きない。CLI が古くてセッション ID を渡せなかった場合などの縮退）。この判定は `Cmd+W`・`Cmd+Option+W`・タブバーの x ボタン・メニューの「タブを閉じる」で共通。何本のプロセスが終わるかは x ボタンのラベル（ポインタを合わせたときの説明と読み上げ名の両方）に出る。
...
**「アプリを閉じても AI の作業を続ける」を有効にしているのに、必要なものが揃っていないとき**も、同じ場所に専用のメッセージが出る。この状態では**タスク一覧のどの行も押せなくなる**（タブに戻す先が無いため）ので、理由が画面に出ていないと「アプリが壊れた」ようにしか見えない（[#244](https://github.com/i-iwnl/ai-terminal/issues/244)）。

> このメッセージが出るのは**設定を有効にしているのに使えないとき**だけ。設定を切っているときは出ない（それは意図した状態であってトラブルではない）。**行ごとには何も出さない** — 全行に同じ文が並ぶと一覧が読めなくなるため、パネルに1つだけ出す。
...
| `Cmd+W` | アクティブなペインを閉じる（**中の AI も終了する**。作業中なら確認が入る。タブに残るペインが1枚だけならタブごと閉じ、最後の1枚を閉じたときと同じく新しいシェルが開く） | ファイル |
...
| （キー無し） | **選択中の AI を終了**（メニューのみ。タスク一覧で最後に選んだ行が対象。対象が無いときは淡色で無効） | ファイル |
...
ホイールで動かせるのは、その画面のなかを表示している CLI（claude / gemini / `less` / `vim` など）自身のスクロールになる。**マウス操作を受け付ける CLI にはホイールをそのまま渡し**（claude / gemini はこちら。CLI 側が自分で転写をスクロールする）、**受け付けない CLI にだけ、回転量を「その行数ぶんの矢印キー」に変換して渡す**（`less` や `set mouse=` していない `vim` はこちら）。後者では `Option` または `Control` を押しながら回すと5倍速、`Shift` を押しながらは横スクロール扱いで送らない。

> ⚠ **claude で `Scroll wheel is sending arrow keys · use PgUp/PgDn to scroll` が出る場合は、[#251](https://github.com/i-iwnl/ai-terminal/issues/251) より前のバージョン。** ホイールが矢印キーに化けて CLI の入力履歴を送っていた。現在はマウス操作として渡している。
