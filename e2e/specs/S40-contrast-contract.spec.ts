...
test('S40 画面のコントラスト比が、記録した値から動いていない', async () => {
  ...
  const mainTargets: ContrastTarget[] = [
    ...
    // --- Issue #139（#180 周1）: ペインヘッダ -----------------------------------
    //
    // **分割中だけフローに入る帯**（`.terminal-pane--split` のときだけ出る）。
    // ここまでで `Meta+d` を押しているので、この時点で存在している。
    //
    // 文字は `--text-secondary`、塗りは `--surface-2`。**どちらも一度も測っていなかった。**
    // `--surface-2` は「サイドバーの行ホバー」として選ばれた値なので、
    // **ターミナル背景の上に置いたときの比は誰も見ていない**（周5-b でメニューが
    // 同じ形の見落としだった: 面のトークンを別の面へ持ち込んで段が潰れる）。
    {
      name: 'ペインヘッダの文字（対ヘッダの塗り）',
      kind: 'text',
      selector: '.pane-header',
      property: 'color',
    },
    {
      // ⛔ **`against` を省かない。** 省くと `effectiveBackground` が自分自身の
      // 不透明な塗りを拾って 1.00 になる（「選択中タブの塗り」と同じ理由）。
      // 比べる相手は**この帯が乗っている面** = ターミナル側。
      name: 'ペインヘッダの塗り（対ターミナル背景）',
      kind: 'non-text',
      selector: '.pane-header',
      property: 'background-color',
      against: '.terminal-stack',
  ...
  const newTabMenu = await measureContrast(window, [
    ...
    {
      // ⚠ **`against` を「選択面の塗り」にしてはいけない**（PR 1 ではそうしていた）。
      // measureContrast は**色だけを比べる**ので、塗りと枠が実際に接しているかを見ない。
      // 塗りを枠から離す是正を入れても **2.65 のまま**で、この行は「離れたこと」を
      // 1ミリも証明しなかった（`spec-writing-traps.md` の「測る対象そのものが違う」）。
      //
      // ここで測るのは「**枠が、いま枠に接している面に対して見えるか**」。
      // 接していないことのほうは幾何で見る必要があるので **S64 が担当する**。
      name: 'メニューの枠（対メニュー面）',
      kind: 'non-text',
  ...
  const expected: Record<string, { ratio: number; wcag: 'pass' | 'fail' }> = {
    ...
    // **最前面へ持ち込むと面の段を逆走する**。
    //
    // **PR 2 で 1.08 -> 1.30 に上げた（--surface-menu-active #3a3a3a）。**
    // 上がってもなお 3:1 には届かないので札は `fail` のまま。**これは意図した状態**で、
    // `.tab-bar__tab.is-active` の 1.23 とまったく同じ扱い:
    // **選択状態を担うのは塗りではなく白 2px の線**（対選択面 11.37）。
    // 線は S40 では測れない（box-shadow は色以外の成分を持つ）ので S64 が固定する。
    //
    // ⛔ **塗りだけで 3:1 に届かせようとしない。** 届く明るさ（#717171 以上）にすると
    // --text-primary が 4.5:1 を割り、文字色も一緒に動かす羽目になる
    // （`design-rules.md` の却下済みの案）。
    'メニュー項目の選択面の塗り（対メニュー面）': { ratio: 1.3, wcag: 'fail' },
    // メニューの唯一の境界（端末出力の上に浮くので、ここからがメニューだと分かる情報を運ぶ）。
    // **枠に接しているのはメニュー面。** 現在項目の塗りは水平 padding のぶん離してある
    // （接していると 2.65 で 3:1 を割る）。**接していないことの検査は S64。**
    'メニューの枠（対メニュー面）': { ratio: 3.43, wcag: 'pass' },
    ...
    // **Issue #164（#179 周2）で 4.47 -> 13.58 に是正した。値は1つも変えていない。**
    // `.tab-bar__tab.is-exited` に `:not(.is-active)` を付けて詳細度を (0,3,0) にし、
    // 選択中タブでは `.is-active` の `--text-bright` が生きるようにしただけ。
    // バッジは `color` の宣言を落として継承させたので、タイトルと必ず同じ色になる。
    // **札も 'fail' -> 'pass' に直す。** ここを直し忘れると staleFail が落とす
    // （そして 'pass' にした時点で、下の閾値ループの監視対象に入る）。
    '終了したタブの文字（選択中・対 --surface-tab-active）': { ratio: 13.58, wcag: 'pass' },
    '終了バッジの文字（選択中・対 --surface-tab-active）': { ratio: 13.58, wcag: 'pass' },
    ...
    // Issue #139（#180 周1）: ペインヘッダ。**一度も測っていなかった2箇所。**
    // この周では値を1つも変えないので、実測をそのまま記録する。
    //
    // 文字は 6.69 で 4.5:1 を満たす（`--text-secondary` が `--surface-2` の上に乗る）。
    //
    // **塗りは 1.05。** `--surface-2`(#222222) をターミナル背景(#1e1e1e)の上に置いた値で、
    // **帯としてはほぼ見えていない**。ただしこれは「選択中タブの塗り 1.23」と同じ型で、
    // **帯の境界は必要な情報を運んでいない**（分割中かどうかは2枚のペインが見えることで、
    // 種別と cwd はヘッダの**文字**が担う）。塗りを明るくすると
    // **ターミナルの最初の行の隣に明るい帯が走る**ことになり、#56 で
    // 「ヘッダがターミナルの1行目を隠した」を直した判断と衝突する。
    // **承知で fail のまま固定する。**
    'ペインヘッダの文字（対ヘッダの塗り）': { ratio: 6.69, wcag: 'pass' },
    'ペインヘッダの塗り（対ターミナル背景）': { ratio: 1.05, wcag: 'fail' },
    // 設定ウィンドウ（最も明るい面）
