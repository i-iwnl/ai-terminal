...
test('S40 画面のコントラスト比が、記録した値から動いていない', async () => {
  ...
  const expected: Record<string, { ratio: number; wcag: 'pass' | 'fail' }> = {
    // 本体ウィンドウ
    'メタ情報の文字（サイドバー上）': { ratio: 3.93, wcag: 'fail' },
    '状態ラベルの文字（あなたの番 / 作業中）': { ratio: 9.16, wcag: 'pass' },
    'CLI の生の値の文字（busy / idle）': { ratio: 5.44, wcag: 'pass' },
    '一覧の主タイトルの文字': { ratio: 12.94, wcag: 'pass' },
    '非選択タブの文字': { ratio: 6.11, wcag: 'pass' },
    // **この行が Issue #20 の核心。選択状態が塗りだけで表現され、その塗りが見えない**
    '選択中タブの塗り（対タブバー）': { ratio: 1.15, wcag: 'fail' },
    // 1.71 ではない。枠が乗っているのは検索バーの浮いた面（--surface-float #2a2a2a）で、
    // ターミナル背景（#1e1e1e）ではない。**紙の上の表は背景を取り違えていた**
    '検索欄の枠（唯一の境界）': { ratio: 1.47, wcag: 'fail' },
    '作業中のドット（対ホバー面）': { ratio: 5.72, wcag: 'pass' },
    // 設定ウィンドウ（最も明るい面）
    // 1.50 ではない。枠の外側は設定ウィンドウの面（--surface-3 #232323）で、
    // 入力欄自身の塗り（#1c1c1c）と比べると 1.50 になる。**低いほうが実際の見え方を決める**
    '設定の入力欄の枠（唯一の境界）': { ratio: 1.38, wcag: 'fail' },
    '設定の注記の文字': { ratio: 3.51, wcag: 'fail' },
    '設定の見出しの文字': { ratio: 5.52, wcag: 'pass' },
    '設定の値の文字': { ratio: 9.79, wcag: 'pass' },
  };
