...
test('S31 設定パネルから変更するとターミナルに反映される', async () => {
  ...
  const soundSelect = dialog.locator('select[aria-label="音の種類"]');
  await expect(soundSelect).toHaveValue('');

  // フォントサイズを既定（13）と違う値に変える。
  //
  // **`input[type="number"]` の `.first()` で引かないこと。** 数値入力は
  // 「外観 > サイズ」と「動作中の AI > 更新間隔（ミリ秒）」の2つあり、`.first()` が
  // 前者を指すのは「外観 が最初のセクションだから」という暗黙の DOM 順序への
  // 依存でしかない（PR 13-b で節を並べ替えたとき、実際に「動作中の AI」が
  // 2番目に上がって順序が動いた）。セクションを並べ替えると更新間隔のほうに化け、
