 ...
 * **判定できる関門が1本も無かった**:
 *
 * - セクション見出し（`表示` / `通知` / `Slack / Discord への転送` /
 ...
 * `'動作' -> 'エージェント'` のような変化が現れ、**それがレビュー資料になる**。
...
test('S70 設定ウィンドウの見出しと項目ラベルが、記録した文字列から動いていない', async () => {
  ...
  //   - ラベル + 入力欄の行 -> `.settings__label`（例: フォント / ポーリング間隔(ms)）
  //   - チェックボックスの行 -> クラスの無い <span>（例: 通知音を鳴らす）
  ...
  const expected: SettingsSection[] = [
    { heading: '表示', labels: ['フォント', 'サイズ'] },
    {
      heading: '通知',
      labels: ['タスクの完了時に通知する', '通知音を鳴らす', 'サウンド'],
    },
    {
    ...
    },
    {
      heading: '動作',
      labels: [
        'tmux が使えるなら AI CLI をラップする',
        'タスク一覧を現在のディレクトリに絞り込む',
        'ポーリング間隔(ms)',
      ],
    },
  ];
  ...
  //
  //    `input[type="number"]` は「サイズ」と「ポーリング間隔(ms)」の2つあり、
  ...
  await expect(dialog.getByLabel('ポーリング間隔(ms)')).toHaveAttribute('type', 'number');
});
