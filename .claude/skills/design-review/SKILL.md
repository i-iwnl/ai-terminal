...
**判定に迷ったら「画素が動くか」で切る。** 実例2件:

| 周 | 判定 | 理由 |
|---|---|---|
| 設定に「いま有効です」の1行を出す（#149） | **回した** | 画面に文字が増える。**案の中核が5箇所覆った** |
| 入力欄のアクセシブル名を変える（#150） | **回さなかった** | `aria-label` だけで**画素は1つも動かない**（`make e2e-screenshots-check` が画素差 0 で裏付け） |

⛔ **「a11y に関係するから回す」にしない。** 支援技術向けの規約は
[reference/implementation-traps.md](reference/implementation-traps.md) に確定済みのものがあり、
**それを適用すれば足りる周がある**。
