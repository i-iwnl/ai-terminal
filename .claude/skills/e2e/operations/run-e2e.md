...
### flaky が1件出るのは、たいてい起動そのものの失敗

`firstWindow: Timeout ... waiting for event "window"` で落ちて、リトライで緑になるもの。**テストの書き方の問題ではないので、待ちを足しても直らない。**

実測:

| 測った時期 | 起動数 | 失敗 | 率 |
|---|---|---|---|
| Issue #17（フル実行3回） | 99 | 2 | **2.0%** |
| Issue #120 周6（`make e2e` フル実行1回。撮影レーンを含む） | 94 | 6 | **6.4%** |
| Issue #121 周4（撮影レーンのフル実行を約11回） | 約 143 | 5 | **約 3.5%** |

| | 値 |
|---|---|
| `firstWindow` の所要時間（成功時） | 最小 101ms / 中央 119ms / 最大 421ms |
| 失敗時の挙動 | Electron のプロセスは立つが、**60 秒待ってもウィンドウが出ない** |

**率は 2〜6% の幅で動く。** マシンの負荷に応じて上下し、Issue #120 周6 の 6.4% は
CPU を長時間埋めた直後の実行だった。**「1件までは既知」という固定の閾値ではなく、
数%のオーダーで出るものとして扱う。**

分布が二極なので、**上限を伸ばしても成功率は上がらない**（判明が遅れるだけ）。特定のシナリオに寄らず毎回別の spec で起きるため、落ちた spec を疑っても手掛かりにならない。

**受け入れる判断（Issue #121 B-3）。** 切り分けの試み（起動間隔を空ける / 1プロセスあたりの
起動数を変える）は行わず、**リトライで吸収する前提を全レーンに揃える**ことで閉じた。
根拠:

- 失敗は `firstWindow` が返らない形で、**アプリ側のコードが1行も動いていない**
  （ウィンドウが出ないので Renderer も起動していない）。アプリの不具合として
  追える手掛かりが無い
- 所要時間の分布が二極（成功は最大 421ms、失敗は 60 秒待っても出ない）で、
  **待ちを伸ばしても救えない**ことが実測で分かっている
- `retries: 1` で全部 green になる（本物の失敗は2回とも落ちるので見逃さない）

**やったのは「取りこぼす経路を無くす」ことだけ。** 撮影レーン
（`e2e/screenshots.playwright.config.ts`）には retries が無く、flake が1回起きると
**画像が1枚欠けたまま終わっていた**。Issue #121 周4 で `retries: 1` に揃えた。

**タイムアウトの予算は連動している。** `firstWindow` の 15 秒（`harness.ts`）＋ 後始末の猶予 5 秒（`CLOSE_GRACE_MS`）が、`playwright.config.ts` の `timeout: 30_000` に収まっていること。収まらないと、後始末の途中でテストが打ち切られ、**リトライで緑になっても Playwright が「どのテストにも属さないエラー」（Worker teardown timeout）を出して `make e2e` が失敗する**（実際に起きた）。片方だけを縮めないこと。
