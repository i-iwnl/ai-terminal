...
- [x] ~~周2-g では `aria-label` への追加だけ行う~~ -> **不可能だった。** 押せない行は `<div>` で
  アクセシブル名を持たないので、`aria-label` だけでは届かない（S106 が捕まえた）。
  メタ行の先頭（`basename(cwd)` の前）に4〜5文字で出す形を周2 で入れた
- [ ] **実測: 待ち行だけメタ行が2行になり、行高が 55px -> 73px（+18px）。** 1週間使ってから、
  この置き場所のままにするか決める（他の対案は worklog の表）
