...
| 実装 | 進行中（Phase 0 完了、Phase 1 は PR 4 まで完了 = **トークンへの置換は完了**） |
| 検証 | 各 PR で完了 |
...
| 優先度 | アクション | 詳細 |
|---|---|---|
| **P0** | PR 5（**値の変更**）に着手する | 見た目が動く唯一の周。着手前に `/design-review` を通す。値の根拠は Issue #20 の A-3 の実測表。**`make css-substitution-check` は落ちるのが正しい**。画像12枚の撮り直しが関門 |
| P1 | PR 4 のコミット / PR 作成 | ブランチ `refactor/css-tokens-type-and-space` に未コミットで置いてある（ユーザーの指示待ち） |
