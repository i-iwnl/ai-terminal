...
## 2026-08-12 - 周8: 利用者が実機で踏んだ不具合（タブが開いている行を終了してもタブが残る）

⚠ **#244 をマージ・インストールしたあと、利用者が実機で踏んだ。**
「終了は押せますがタブは閉じません」。

### ⛔ 原因は、私（メイン）のワーカーへの指示ミス

計画ゲートでは**正しく判断していた**:

> `'focus'` の行はタブが開いているので既存の「タブを閉じる」が担当します。
> **つまり `agentSessionKill` に意味があるのは `'recover'` の行だけ**です。

ところが**ワーカーへの指示には「押せる行にだけ `onContextMenu` を付ける」としか書かなかった。**
`clickable = action !== 'none'` なので、**押せる行には `'focus'` も含まれる**。

結果、タブが開いている行を右クリックして「この AI を終了」を押すと:

1. `agentSessionKill` が tmux セッションを終了させる
2. タブの中の PTY（tmux クライアント）も終わる
3. ⛔ **タブは「プロセスは終了しました」の状態で開いたまま残る**（Renderer は何も閉じていない）

⭐ **「計画で気づいていたことを、委譲するときに落とした」** のが本質。
周6-a の教訓「関門を作らせなかったのは私の分解ミス」と**同じ形の再発**。

### なぜ E2E で捕まらなかったか

**`'focus'` の行を右クリックする spec が1本も無かった。** S113 / S114 / S115 / S116 は
すべて「AI を残してタブを閉じる」-> `tmux-live-panes.txt` で生存させる、という手順を通るので
**右クリックする行は常に `'recover'`**。loop.md の「**経路が2つあるのに片方向しか踏んでいない**」型。

### 直し方（design-rules から導いた。ユーザーに聞いていない）

根拠は `design-rules` §6「**行き先が複数に分かれている操作は、文言で『どちらか』まで言い切る**」。

| 行の状態 | ラベル | 起きること |
|---|---|---|
| `'recover'`（タブが無い） | `この AI を終了` | `agentSessionKill`（従来どおり） |
| `'focus'`（タブが開いている） | **`このペインを閉じて AI を終了`** | **そのペインを閉じる**（`ptyKill` の `terminateSession: true` が tmux も終了させる） |

- ⭐ **`'focus'` では `agentSessionKill` を呼ばない。** ペインを閉じる経路が既に tmux を終了させる（二重にしない）
- **`useTabs.ts` に `closePaneForAgentSession()` を新設。** `findPaneByAgentSessionId`（`onFocusTab` が既に使っている引き当て）を再利用し、
  **最後の1枚なら `closeTab` に合流**する
- ⛔ **`closeActivePane` を汎用化しなかった。** あれは `activeTabIdRef` 前提で、
  「対象が非アクティブなこともある」分岐を割り込ませると **`Cmd+W` の挙動まで変わる**
- **メニューバーの「選択中の AI を終了」も同じ不具合を持っていた**が、
  両導線が `runAction` -> `requestKillSession` -> `performKillSession` の**1本に集約**されていたので、
  そこの分岐1つで同時に直った。ラベルは常設項目なので据え置き
- **busy 確認は `requestKillSession` が唯一のゲートのまま**（1行も触っていない）

### 関門（S118 新設）

| 壊し方 | 出た赤 |
|---|---|
| 修正前（ラベルの出し分けが無い） | `メニューに「このペインを閉じて AI を終了」が無い（実際の項目: この AI を終了）` |
| ⭐ **ラベルだけ直して挙動は元のまま**（= 利用者が踏んだ状態そのもの） | `toHaveCount` `Expected: 0 / Received: 1`（**タブが1枚残っている**） |
| `hasOpenTab` を常に true（タブが無い行のラベルまで変える） | `メニューに「この AI を終了」が無い（実際の項目: このペインを閉じて AI を終了）` |

⭐ **2番目が本体。** 1番目（ラベル）だけだと「ラベルは正しいがタブが残る」実装で緑になる。

### ⚠ 実機で確認できなかったこと

**実機では `'focus'` の行を安全に作れなかった。**

- 偽 `claude`（`sleep 900`）で tmux セッションを作る手は使えるが、**それは `claude agents --json` に出ない**ので、
  「タブに戻せる AI」節の行をクリックしてアタッチすると**一覧から消えるだけ**で `'focus'` の行にならない
- `'focus'` の行を作るには「`agents --json` に出ていて、かつタブが開いている」が要る。
  実機でそれを満たすのは**利用者が実際に使っているセッション**だけ。⛔ **触らない**
- E2E は `setAgentEntries()` で `agents --json` を偽装できるので作れる（S104 と同じ手）

**つまりこの修正の確認手段は S118 が唯一。** 実機との差は「`agents --json` を偽装できるか」。

### 教訓

- ⭐⭐ **計画で気づいた除外条件は、委譲するときにワーカーへ渡さないと消える。**
  周6-a では「関門を作らせなかった」、周8 では「除外条件を伝えなかった」。
  **2回とも、私が知っていたのに指示に書かなかった。**
  -> **ワーカーへの指示は「何を作るか」だけでなく「何を対象外にするか」を必ず書く**
- ⚠ **`AI_TERMINAL_DATA_DIR` に渡すディレクトリを `mkdir -p` し忘れると、
  アプリは起動するが CDP のターゲットが0件になる**（`DevTools listening` は出る）。
  verify-on-device.md は `mkdir -p "$D"` と書いてある。**省くと「アプリが壊れた」に見える**
- ⚠ **実機の tmux セッションを片付けるときは、自分が作った UUID と突き合わせること。**
  `aiterm-` が付いているからといって自分のものとは限らない
  （利用者の実セッションが `(attached)` で残っていた）

### 検証

- `make check` 緑（**859件**）/ `make e2e-lint` **FAIL=0**（PASS=943）
- `npx playwright test S118 S104 S105 S107〜S117` **14本すべて green**
- ⚠ **フル `make e2e` は未実施 / 未コミット**（ブランチも切っていない）

### 次に再開するとき最初に読むべきこと

- ⛔ **`main` の作業ツリーに未コミットの修正がある。** ブランチを切ってから出す
- 出す前に**フル `make e2e`** と `make e2e-lint`。UI 文言を変えたので `make e2e-screenshots` も判定する
- **インストール済みの `/Applications/ai-terminal.app`（2026-08-12 16:52）はこの修正を含まない。**
  マージ後に `make install-app` をやり直すこと
